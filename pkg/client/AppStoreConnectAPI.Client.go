...
// AppStoreConnectAPIClient is a wrapper for the HTTP client with middleware and configuration
type AppStoreConnectAPIClient struct {
...
// NewAppStoreConnectAPIClient creates a new API client with default middlewares
func NewAppStoreConnectAPIClient(config *Config) *AppStoreConnectAPIClient {
...
func (c *AppStoreConnectAPIClient) Use(middleware Middleware) {
	c.middlewares = append(c.middlewares, middleware)
}

// LoggingMiddleware adds debug logs for requests and responses
func (c *AppStoreConnectAPIClient) LoggingMiddleware() Middleware {
...
func (c *AppStoreConnectAPIClient) Do(req *http.Request) (*http.Response, error) {
	// Apply middlewares
...
func (c *AppStoreConnectAPIClient) RetryMiddleware(maxRetries int, delay time.Duration) Middleware {
	return func(next Handler) Handler {
		return func(req *http.Request) (*http.Response, error) {
...
func (c *AppStoreConnectAPIClient) Get(endpoint string, headers map[string]string, params any) ([]byte, int, error) {
...
func (c *AppStoreConnectAPIClient) Post(endpoint string, body []byte, headers map[string]string) ([]byte, int, error) {
	url := fmt.Sprintf("%s/%s", c.Config.BaseURL, endpoint)
	if headers == nil {
		headers = make(map[string]string)
	}
...
func (c *AppStoreConnectAPIClient) PUT(endpoint string, headers map[string]string, body []byte) ([]byte, int, error) {
	url := fmt.Sprintf("%s/%s", c.Config.BaseURL, endpoint)
	if headers == nil {
		headers = make(map[string]string)
	}
...
func (c *AppStoreConnectAPIClient) Patch(endpoint string, headers map[string]string, parameters any) ([]byte, int, error) {
	url := fmt.Sprintf("%s/%s", c.Config.BaseURL, endpoint)
	if headers == nil {
		headers = make(map[string]string)
	}
...
func (c *AppStoreConnectAPIClient) Delete(endpoint string, headers map[string]string, body []byte) ([]byte, int, error) {
	url := fmt.Sprintf("%s/%s", c.Config.BaseURL, endpoint)
	if headers == nil {
		headers = make(map[string]string)
	}
...
func (c *AppStoreConnectAPIClient) GenerateAuthorizationJWT(method string, endpoint string) string {
